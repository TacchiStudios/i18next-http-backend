...
class Backend {
  ...
  async loadUrl(url, callback, languages, namespaces) {
    const cacheKey = 'i18next-cache-' + url;
    const cacheResultString = await AsyncStorage.getItem(cacheKey);

    if (cacheResultString) {
      let cacheResultJSON;
      try {
        cacheResultJSON = JSON.parse(cacheResultString);
      } catch (error) {
        await AsyncStorage.removeItem(cacheKey);
      }

      if (cacheResultJSON) {
        const { data, timestamp: cacheTimestamp } = cacheResultJSON;
      
        callback(null, data);

        const CACHE_TIMEOUT_IN_MS = 60 * 60 * 1000; // 1 hour
        const msSinceCacheSet = Date.now() - cacheTimestamp;
        const isCacheStillFresh = msSinceCacheSet <= CACHE_TIMEOUT_IN_MS;
        if (isCacheStillFresh) {
          // Don't update the strings from the backend server
          return;
        }
      }
    }

    this.options.request(this.options, url, undefined, async (err, res) => {
      if (res && ((res.status >= 500 && res.status < 600) || !res.status)) return callback('failed loading ' + url, true /* retry */)
      if (res && res.status >= 400 && res.status < 500) return callback('failed loading ' + url, false /* no retry */)
      if (!res && err && err.message && err.message.indexOf('Failed to fetch') > -1) return callback('failed loading ' + url, true /* retry */)
      if (err) return callback(err, false)

      let data, parseErr
      try {
        if (typeof res.data === 'string') {
          data = this.options.parse(res.data, languages, namespaces)
      ...
      const cacheContent = { data, timestamp: Date.now() };
      await AsyncStorage.setItem(cacheKey, JSON.stringify(cacheContent));
